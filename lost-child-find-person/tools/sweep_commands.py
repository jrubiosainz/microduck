...
def rollout(model, policy, twist, seconds: float) -> dict:
    ...
    # Displacement in the duck's OWN starting frame: forward and left.
    ...
    radius = (float(np.linalg.norm(displacement)) / (2.0 * abs(math.sin(
        0.5 * yaw_delta))) if abs(yaw_delta) > math.radians(4.0) else float("inf"))
    return {
...
def main() -> int:
    ...
    for vx in (0.14, 0.18, 0.20, 0.22, 0.26, 0.30, 0.34, 0.38, 0.42, 0.46, 0.52):
    ...
    print("=" * 92)
    print(f"{'vx':>6} {'wz':>6} {'yaw°':>8} {'°/s':>8} {'R m':>9} "
          f"{'net m':>8} {'min z':>7} {'fall':>5}")
    for vx in (0.0, 0.30, 0.38):
        for wz in (-0.55, -0.42, -0.30, -0.22, -0.16, 0.16, 0.22, 0.30, 0.42, 0.55):
            r = rollout(model, policy, (vx, 0.0, wz), args.turn_seconds)
            radius = r["turn_radius_m"]
            radius_text = "    inf" if math.isinf(radius) else f"{radius:9.3f}"
            print(f"{vx:6.2f} {wz:6.2f} {r['yaw_deg']:8.1f} "
                  f"{r['yaw_rate_dps']:8.1f} {radius_text} "
                  f"{r['net_m']:8.3f} {r['min_z']:7.3f} {r['fallen_steps']:5d}")

    print()
    print("=" * 92)
    print("COAST — distance travelled after the command goes to EXACT zero")
    print("=" * 92)
    for vx in (0.30, 0.38, 0.46):
        data = mujoco.MjData(model)
        mujoco.mj_resetData(model, data)
        runner = policy.reset(model, data)
        trunk = model.body("trunk_base").id
