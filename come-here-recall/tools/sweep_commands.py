#!/usr/bin/env python3
"""Measure stock-policy locomotion commands on scene_come_here_recall.xml.

Every constant used by the behavior must be MEASURED on this scene with this
model, because the forward gait has a hard onset threshold: a small command
does not give small motion, it gives NO motion.  PR #22 measured its constants
against ``robot_allcollisions.xml``; this scene is built on ``robot_walk.xml``,
so the numbers are re-measured here rather than inherited.

What THIS behavior needs, and therefore what the grid covers:

* forward approach speeds, to find the slowest command that reliably crosses
  gait onset (a recall must be able to close a 2 m gap without sprinting);
* forward + yaw, because the duck has to curve toward a caller who is off to
  one side while still making ground;
* turn-in-place in both signs, measured separately, because the stock policy is
  strongly asymmetric and a named left/right cannot be trusted;
* a slow "final metres" command, since the standoff band is only 0.30 m wide
  and overshooting it fails the acceptance gate.
...
DEFAULT_GRID = [
    # forward gait onset and cruise
    (0.16, 0.0, 0.0), (0.20, 0.0, 0.0), (0.24, 0.0, 0.0), (0.26, 0.0, 0.0),
    (0.28, 0.0, 0.0), (0.32, 0.0, 0.0), (0.36, 0.0, 0.0), (0.42, 0.0, 0.0),
    # turn in place, both signs measured independently
    (0.0, 0.0, 0.45), (0.0, 0.0, -0.45),
    (0.0, 0.0, 0.60), (0.0, 0.0, -0.60),
    (0.0, 0.0, 0.85), (0.0, 0.0, -0.85),
    # curve toward a caller while making ground
    (0.24, 0.0, 0.45), (0.24, 0.0, -0.45),
    (0.24, 0.0, 0.60), (0.24, 0.0, -0.60),
    (0.28, 0.0, 0.45), (0.28, 0.0, -0.45),
    (0.28, 0.0, 0.60), (0.28, 0.0, -0.60),
    (0.28, 0.0, 0.85), (0.28, 0.0, -0.85),
    (0.32, 0.0, 0.60), (0.32, 0.0, -0.60),
    # gentle heading trim while approaching
    (0.28, 0.0, 0.25), (0.28, 0.0, -0.25),
    (0.28, 0.0, 0.35), (0.28, 0.0, -0.35),
    # slow final metres into the standoff band
    (0.20, 0.0, 0.0), (0.22, 0.0, 0.0),
]
