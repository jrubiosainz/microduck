...
SUMMARY_PATH = Path(os.environ.get("GESTURE_SUMMARY", "/tmp/gr_final.json"))
TRACE_PATH = Path(os.environ.get("GESTURE_TRACE", "/tmp/gr_trace.json"))


@pytest.fixture(scope="session")
def model():
    """The compiled training-area scene."""
...
def summary():
    ...
    if not SUMMARY_PATH.is_file():
        pytest.skip(
            ...
            f"scripts/validate_gesture.py --json {SUMMARY_PATH}")
...
def trace():
    ...
    if not TRACE_PATH.is_file():
        pytest.skip(
            ...
            f"scripts/validate_gesture.py --trace {TRACE_PATH}")
...
def short_rollout():
    ...
    rollout = GestureRollout(str(POLICY), 3.0)
    rollout.run()
    return rollout
