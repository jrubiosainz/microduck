...
def main() -> int:
    ...
    parser.add_argument("--what", default="all",
                        choices=("all", "forward", "reverse", "yaw",
                                 "ceiling", "lateral", "spin", "zero",
                                 "turn"))
    ...
    if args.what in ("all", "reverse"):
        print()
        print("=" * 78)
        print("REVERSE  - THE measurement the BACK_UP command turns on.")
        print("Backward gait onset is a SEPARATE cliff from the forward one and")
        print("sits much deeper; a small negative vx produces NO motion at all.")
        print("=" * 78)
        print(f"  {'vx':>6} {'net m':>8} {'m/s':>8} {'yaw deg':>9} "
              f"{'back m':>8} {'min z':>7}")
        for vx in (-0.16, -0.22, -0.26, -0.30, -0.32, -0.34, -0.38, -0.42,
                   -0.48, -0.54):
            runner, data, dec = fresh(policy, model)
            trunk = model.body("trunk_base").id
            start = data.xpos[trunk][:2].copy()
            start_yaw = runner.yaw(data)
            r = run(runner, model, data, (vx, 0.0, 0.0), args.seconds, dec)
            delta = data.xpos[trunk][:2].copy() - start
            # Displacement along the duck's OWN pre-command heading.  Negative
            # means it genuinely went backwards relative to where it was
            # facing, which is the only sense in which "it backed up" is a
            # claim rather than a label.
            back = float(delta[0] * math.cos(start_yaw)
                         + delta[1] * math.sin(start_yaw))
            print(f"  {vx:6.2f} {r['net_m']:8.3f} {r['speed_mps']:8.3f} "
                  f"{r['yaw_deg']:9.1f} {back:8.3f} {r['min_z']:7.4f}")

    if args.what in ("all", "turn"):
        print()
        print("=" * 78)
        print("TURN ARCS  - what a commanded LEFT or RIGHT actually delivers.")
        print("Measured as the TRUNK YAW DELTA over a fixed walk, per sign,")
        print("because this behavior's whole claim is that its left and right")
        print("are opposite PHYSICAL heading changes rather than two labels.")
        print("=" * 78)
        print(f"  {'vx':>6} {'wz':>6} {'secs':>6} {'yaw deg':>9} "
              f"{'deg/s':>8} {'net m':>8} {'radius m':>9} {'min z':>7}")
        for vx in (0.26, 0.30):
            for wz in (-0.58, -0.50, 0.50, 0.58):
                for seconds in (4.0, 6.0):
                    runner, data, dec = fresh(policy, model)
                    r = run(runner, model, data, (vx, 0.0, wz), seconds, dec)
                    rate = abs(r["yaw_dps"])
                    radius = (r["path_m"] / seconds) / math.radians(
                        max(rate, 1e-6))
                    print(f"  {vx:6.2f} {wz:6.2f} {seconds:6.1f} "
                          f"{r['yaw_deg']:9.1f} {r['yaw_dps']:8.1f} "
                          f"{r['net_m']:8.3f} {radius:9.3f} {r['min_z']:7.4f}")
