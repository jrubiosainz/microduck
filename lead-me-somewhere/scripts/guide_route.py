...
* the **crowd actors** walk these routes, so their heading is continuous and a
  linear prediction of where they will be in three seconds is worth making;
* the **planned guide route** is turned into one of these, so "the duck led her
  along a route with three bends" is a statement about a path with a defined
  tangent everywhere, and the bends can be counted by
  :meth:`Route.corner_report` instead of eyeballed.
