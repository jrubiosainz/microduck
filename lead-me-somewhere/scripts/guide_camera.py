#!/usr/bin/env python3
"""What the duck can actually see of the person it is leading.
...
of truth.  The ARRIVAL GESTURE goes through the same isolation: it is a head
motion in the rendering copy only, which is what makes it body-safe by
construction rather than by restraint.

Visibility is measured from the EXACT camera the PiP would render from, so the
reported percentages and any future picture agree.  The test is real camera
geometry — frustum containment plus an occlusion ray cast through actual scene
geometry — but the IDENTITY of each body comes from the simulator.  That is a
**semantic proxy** for person recognition, not RGB classification, and it is
labelled as such wherever it surfaces.

WHY VISIBILITY MATTERS FOR THIS BEHAVIOR SPECIFICALLY
------------------------------------------------------
A guide walks with its back to the person it is leading.  That is the whole
difficulty: the follower is BEHIND the robot for the entire route, at the far
edge of a forward-facing head's reach, and the only way to keep her accounted
for is to keep the head turned.  The gate therefore requires her to be visible in
almost every tick of CHECK_FOLLOWER and WAIT_FOR_PERSON where line of sight
geometrically EXISTS — which is a stricter claim than raw visibility, because it
excludes the ticks where a partition was genuinely in the way and holds the duck
responsible for all the rest.

The interesting case is the second lag episode.  She is behind the partition
there, so line of sight does NOT exist, the camera correctly cannot see her, and
the duck stops BECAUSE it cannot see her.  A gate that demanded visibility in
those ticks would be demanding the impossible; a gate that ignored them would
miss the episode entirely.  Conditioning on line of sight is what separates the
two.
"""
...
from guide_cast import ALL_NAMES, BY_NAME
...
# and any future render must measure through the same frustum.
PIP_W, PIP_H = 300, 216


class GuideCamera:
    """Head camera: track the follower and report per-sample visibility."""
    ...
    def __init__(self, model, data, qpos_idx, trunk_id,
        ...
        self.guide_cam = model.camera("guide_camera").id
        rig = model.body("guide_rig")
        ...
        # Arrival-gesture offsets, applied to the RENDERING head pose only.
        self.gesture_yaw = 0.0
        self.gesture_pitch = 0.0

        half_v = math.radians(float(model.cam_fovy[self.guide_cam])) * 0.5
    ...
    def _pose_head(self, data, duck_yaw: float) -> None:
        ...
        self.gaze_yaw = float(np.clip(
            wrap_angle(self.view_yaw - duck_yaw) + self.gesture_yaw,
            yaw_lo + 0.03, yaw_hi - 0.03))
        self.gaze_pitch = float(np.clip(
            -self.view_pitch + self.gesture_pitch,
            pitch_lo + 0.04, pitch_hi - 0.04))
    ...
    def set_gesture(self, elapsed: float | None) -> None:
        """The arrival gesture: a slow alternating head sweep, RENDERING ONLY.

        ``elapsed`` is seconds into INDICATE, or ``None`` when not indicating.
        Applied as an offset to the rendering head pose, exactly like the gaze,
        so the walking policy never sees it and the gesture cannot destabilise
        the robot.  Its amplitude is inside the measured head joint range.
        """
        if elapsed is None:
            self.gesture_yaw = 0.0
            self.gesture_pitch = 0.0
            return
        phase = 2.0 * math.pi * INDICATE_HZ * elapsed
        self.gesture_yaw = math.radians(
            INDICATE_YAW_AMPLITUDE_DEG) * math.sin(phase)
        self.gesture_pitch = math.radians(
            INDICATE_PITCH_AMPLITUDE_DEG) * math.sin(2.0 * phase)
    ...
    def _visible_samples(self, name: str) -> tuple[list[bool], float, float]:
        ...
        partition.
        """
        eye = self.render_data.cam_xpos[self.guide_cam].copy()
        rotation = self.render_data.cam_xmat[self.guide_cam].reshape(3, 3)
    ...
    def update(self, data, *, duck_yaw: float, subject: str,
               look_at=None, gesture_elapsed: float | None = None) -> dict:
        """Pose the camera for this tick and measure what it actually sees.

        ``subject`` is the body the head tracks — always the follower in this
        behavior, because a guide that stopped watching the person it is leading
        has stopped guiding.  ``look_at`` overrides the aim point, which is how
        the head faces the destination during INDICATE while the follower is
        still the body whose visibility is graded.
        """
        self.set_gesture(gesture_elapsed)
        ...
        return {
            ...
            "gesture_yaw_deg": math.degrees(self.gesture_yaw),
        }

    @property
    def camera_id(self) -> int:
        return self.guide_cam
