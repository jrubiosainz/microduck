#!/usr/bin/env python3
"""What the duck can actually see of the person it is walking beside.
...
------------------------------------------------------
A companion robot that walks beside somebody it cannot see is dead reckoning.
The gate therefore requires the guardian to be visible in almost every tick
where line of sight geometrically EXISTS — which is a stricter claim than raw
visibility, because it excludes the ticks where a column was genuinely in the
way and holds the duck responsible for all the rest.

The interesting case is the crossing.  While the duck is astern of her, she is
at the edge of the frame and the head has to work; while it is beside her she is
almost abeam, which is the hardest sustained aiming problem in the run.  Both are
measured against the same camera, and the LOS-conditioned fraction is what the
gate grades.
"""
...
class BesideCamera:
    """Head camera: track the guardian and report per-sample visibility."""
    ...
    def __init__(self, model, data, qpos_idx, trunk_id,
        ...
        self.beside_cam = model.camera("beside_camera").id
        rig = model.body("beside_rig")
        ...
        half_v = math.radians(float(model.cam_fovy[self.beside_cam])) * 0.5
    ...
    def _visible_samples(self, name: str) -> tuple[list[bool], float, float]:
        ...
        column.
        """
        eye = self.render_data.cam_xpos[self.beside_cam].copy()
        rotation = self.render_data.cam_xmat[self.beside_cam].reshape(3, 3)
    ...
    # -- public ----------------------------------------------------------
    def update(self, data, *, duck_yaw: float, subject: str) -> dict:
        """Pose the camera for this tick and measure what it actually sees.

        ``subject`` is the body the head tracks — always the guardian in this
        behavior, because the duck never stops accompanying her.
        """
