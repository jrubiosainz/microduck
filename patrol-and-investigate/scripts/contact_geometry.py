...
class ContactProbe:
    ...
    def __init__(self, model: mujoco.MjModel, trunk_id: int,
                 person_names: tuple[str, ...], prefix: str = "person_"):
        self.model = model
        self.duck_geoms = geoms_of(model, body_subtree(model, trunk_id))
        ...
        for name in person_names:
            geoms = geoms_of(
                model, body_subtree(model, model.body(f"{prefix}{name}").id))
        ...
        self.duck_rbound = {
            g: float(model.geom_rbound[g]) for g in self.duck_geoms}
        self._duck_radii = np.asarray(
            [self.duck_rbound[g] for g in self.duck_geoms], dtype=np.float64)
        self._person_radii = {
            name: np.asarray([float(model.geom_rbound[g]) for g in geoms],
                             dtype=np.float64)
            for name, geoms in self.person_geoms.items()
        }
    ...
    def distance(self, data: mujoco.MjData, name: str,
        ...
        """
        # Cheap conservative broad phase.  Bounding-sphere surface distance is
        # never larger than the real primitive distance, so if every pair is
        # already beyond the cutoff, the expensive analytic narrow phase cannot
        # improve the answer.  This removes ~80k Python calls per 20 ticks in a
        # five-person scene without weakening the contact gate.
        people = self.person_geoms[name]
        duck_pos = data.geom_xpos[self.duck_geoms]
        person_pos = data.geom_xpos[people]
        delta = duck_pos[:, None, :] - person_pos[None, :, :]
        centre = np.linalg.norm(delta, axis=2)
        lower_bound = centre - self._duck_radii[:, None] \
            - self._person_radii[name][None, :]
        if float(np.min(lower_bound)) >= cutoff:
            return cutoff

        best = cutoff
        for person_geom in people:
            for duck_geom in self.duck_geoms:
                distance = primitive_sphere_distance(
