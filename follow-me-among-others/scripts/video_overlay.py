#!/usr/bin/env python3
"""HUD, search-state pipeline and head-camera PiP for crowd following."""
import math
...
def compose(main_rgb, pip_rgb, *, t, total_seconds, state, state_elapsed,
            selection, target, duck_pos, follow, command, camera, min_height,
            completed_cycles):
    ...
    draw.text((12, 8), f"FOLLOW-ME · AMONG OTHERS   t={t:05.2f}s",
              fill=accent, font=font)
    draw.text((12, 31),
              f"SELECTION {selection}/4   TARGET: {target:<5}   STATE: {state}",
    ...
    for idx, name in enumerate(PIPELINE):
        ...
        draw.rounded_rectangle([x, pipe_y, x + 108, pipe_y + 30], radius=7,
                               fill=fill, outline=STATE_RGB[name], width=2)
        draw.text((x + 9, pipe_y + 7), name, fill=(5, 10, 16) if active else
                  STATE_RGB[name], font=small)
        if idx < len(PIPELINE) - 1:
            draw.text((x + 112, pipe_y + 7), "→", fill=(170, 185, 205), font=font)

    # Four target badges persist so the repeated BLUE target is unmistakable.
    targets = ("BLUE", "GREEN", "RED", "BLUE")
    badge_y = height - 36
    for idx, color in enumerate(targets):
        ...
        draw.rounded_rectangle([x, badge_y, x + 124, badge_y + 23], radius=6,
                               fill=fill, outline=outline, width=2)
        text_color = (5, 10, 16) if active else outline
        prefix = "✓ " if finished else ""
        draw.text((x + 8, badge_y + 4), f"{prefix}{idx + 1} {color}",
                  fill=text_color, font=tiny)

    progress_x = 16 + int((width - 32) * min(t / total_seconds, 1.0))
    draw.line([progress_x, height - 7, progress_x, height - 2],
