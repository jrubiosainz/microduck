...
Used for two different things in this behavior, which is why it is its own
module:

* every scripted PERSON walks one of these routes, so their heading is continuous and a
  linear prediction of where they will be in three seconds is worth making;
* the **duck's own route** through the doorway, the lobby and the cabin is one
  of these too, so "it walked a physical route with real bends" is a statement
  about a path with a defined tangent everywhere, and the bends can be counted
  by :meth:`Route.corner_report` instead of eyeballed.
...
# against the duck's MEASURED yaw authority rather than for looks: at a 0.13 m/s
# cruise a 0.62 m radius turns through 0.21 rad/s = 12.0 deg/s, which the duck
# can follow on its weaker (left) yaw sign.  Smaller than the promenade radius
# used by ``walk-beside-me`` because this concourse is tighter and the route has
# to round a partition end rather than sweep a boulevard corner.
CORNER_RADIUS = 0.62
