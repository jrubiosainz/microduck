#!/usr/bin/env python3
"""What the duck can actually see of the people it is yielding to.
...
proxy** for person recognition, not RGB classification, and it is labelled as
such wherever it surfaces.

WHY VISIBILITY MATTERS FOR THIS BEHAVIOR SPECIFICALLY
------------------------------------------------------
Every decision this robot makes is a decision about somebody else's position: to
stop outside a threshold because two people are coming out of it, to hold still
until the last occupant has left a lift, to move off only once the guardian is
through.  A robot that made those decisions without being able to SEE the people
concerned would be guessing, however correct the outcome looked.

So the gate requires the ACTIVE person - whoever the current state is waiting on
- to be visible in almost every tick where line of sight geometrically EXISTS.
Conditioning on line of sight separates two very different failures: a camera
pointing the wrong way, which is the duck's fault, and a body genuinely behind a
jamb, which is not.  The ticks where an occluder is really in the way are
excluded; the duck is held responsible for all the rest.

THE SUBJECT CHANGES WITH THE STATE, AND THAT IS THE ACQUISITION CLAIM
----------------------------------------------------------------------
``etiquette_aim`` decides who the head tracks in each state: the nearer exiter
while yielding at the door, the last occupant still to leave while the lift
empties, the guardian while following her.  The camera does not choose; it aims
where it is told and reports what it saw.  Keeping the choice outside this module
is what lets the acquisition order be unit-tested with no MuJoCo at all.
"""
...
from etiquette_cast import ALL_NAMES, BY_NAME
...
class EtiquetteCamera:
    ...
    def __init__(self, model, data, qpos_idx, trunk_id,
        ...
        self.lobby_cam = model.camera("lobby_camera").id
        rig = model.body("lobby_rig")
        ...
        half_v = math.radians(float(model.cam_fovy[self.lobby_cam])) * 0.5
    ...
    def set_gesture(self, elapsed: float | None) -> None:
        ...
        silent: the sibling ``lead-me-somewhere`` behavior has an arrival
        gesture, and carrying its code here unused - or worse, wiring it in
        without measuring it on this scene - would be shipping unmeasured
        behavior.  A robot standing in a lift should hold still.
        """
        return
    ...
    def _visible_samples(self, name: str) -> tuple[list[bool], float, float]:
        ...
        partition.
        """
        eye = self.render_data.cam_xpos[self.lobby_cam].copy()
        rotation = self.render_data.cam_xmat[self.lobby_cam].reshape(3, 3)
    ...
    def update(self, data, *, duck_yaw: float, subject: str,
        ...
        ``subject`` is the body the head tracks, chosen by ``etiquette_aim``
        from the current state: whoever the duck is waiting on right now.
        ``look_at`` overrides the aim point with a world position, which is how
        the head faces an aperture rather than a person while the duck crosses
        one.
        """
