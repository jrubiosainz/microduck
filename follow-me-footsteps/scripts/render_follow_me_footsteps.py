#!/usr/bin/env python3
"""Render Microduck following the leader's delayed world-space footsteps.

Unlike v1, steering is selected from a 0.65 m spatial trail. The duck reaches
and executes each corner after the person instead of mirroring it immediately.
"""
import argparse
import json
import math
...
def main():
    ...
    parser.add_argument("--xml", default="src/mjlab_microduck/robot/microduck/scene_follow_me_footsteps.xml")
    parser.add_argument("--policy", default="onnx/alpha_walking.onnx")
    parser.add_argument("--seconds", type=float, default=37.0)
    parser.add_argument("--out", default="/tmp/follow-me-footsteps-frames")
    parser.add_argument("--metrics", default="/tmp/follow-me-footsteps-metrics.json")
    ...
    print(f"follow-me-footsteps: {total_steps} steps, lag={TRAIL_DISTANCE:.2f}m, "
    ...
    summary = {
        ...
        "right_delay_s": (
            command_phase_times.get("RIGHT", float("nan"))
            - leader_phase_times.get("RIGHT", float("nan"))
        ),
        "backward_delay_s": (
